...
# matrix of weights for each lifestage to the population density of each species
# (for density-dependence effects)
...
A_t_function <- function(state, iter, log_F, logit_S, alpha_F, alpha_S, gamma, dd_weights) {
  ...
  # get vital rates for each site/species
  F_t <- exp(log_F_t)
  S_A_t <- ilogit(logit_S_A_t)
  S_J_t <- ilogit(sweep(logit_S_A, 2, gamma, "-"))

  # combine into a single matrix such that that each species has adjacent lifestages
