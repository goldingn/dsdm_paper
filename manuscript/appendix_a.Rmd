...
Simultaneous demographic species distribution modelling of multiple species can be achieved by replacing the population transition matrices at each site $\mathbf{A}_i$ with a block-diagonal matrix comprising the population transition matrices for all the species of interest in the diagonal blocks. For a two-species model $\mathbf{A}_i$ therefore has dimension $(M1+M2)$x$(M1+M2)$, where $M1$ and $M2$ are the number of life stages for species 1 and 2 respectively. Since an individual can neither change species or produce offspring of another species, the off-diagonal blocks are all zero. Interactions between these species can be encoded in density-dependent vital rates, with the density of both conspecific and intraspecific populations having an impact.
...
\mathbf{A}_{l,i,t+1} &= \begin{bmatrix} F_{l,i,t} \cdot S_{J_{l,i,t}} & F_{l,i,t} \cdot S_{A_{l,i,t}} \\ S_{J_{l,i,t}} & S_{A_{l,i,t}}  \end{bmatrix}  \\ \nonumber
ln(F_{l,i,t}) &= \mathbf{X}_i \beta_{F_l} + \textstyle\sum_{l'=1}^{L}{\alpha_{F_{l,l'}} \mathbf{N}_{l',i,t}}\\ \nonumber
logit(S_{A_{l,i,t}}) &= \mathbf{X}_i \beta_{S_l} + \textstyle\sum_{l'=1}^{L}{\alpha_{S_{l,l'}} \mathbf{N}_{l',i,t}}\\ \nonumber
logit(S_{J_{l,i,t}}) &= logit(S_{A_{l,i,t}}) - \gamma_l \\ \nonumber
...
As in the biotic interactions, model different genotypes with a separate set of life stages, but fecundity in off-diagonal blocks gives the probability of the offspring having a different genotype to the parent. Different genotypes can therefore have different vital rates.

How to get separate abundances? intrinsic growth rate-derived abundance, multiplied by relative abundance of each of the genotypes.
