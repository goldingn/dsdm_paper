...
By contrast, matrix population models and integral projection models (IPMs) are demographic models of single populations that construct and analyse a Leslie matrix representing key demographic processes13.
...
For each location, these rates were used to generate a 2x2 Leslie matrix and iterated to estimate the intrinsic growth rate $\lambda$.
...
The model specification was completed with a Bernoulli likelihood and non-informative priors, and fitted to 500 presence-absence records from the Breeding Bird Survey of North America in 2015 [@todo].
...
Like other Leslie matrix approaches, the basic DSDMs I have already developed can also be extended to explicitly model three key ecological processes that are poorly captured by existing distribution models: density dependence, dispersal, and biotic interactions.
...
Biotic interactionsbetween species can be incorporated in a DSDM by representing the interacting dynamics of multiple species in a single Leslie matrix, with inter-species density dependence25.
