...
Moreover, models of species distributions are often needed to predict new distributions in response to changes other than the environmental predictors that correlative SDMs rely upon. For instance <the impact of a novel pathogen or the introduction of a competitor, on a species' distribution and abundance.>

Whilst users typically have some knowledge of the processes driving these aspects of species distributions &mdash; such as the species' life history, approximate survival and fecundity rates or dispersal ability &mdash; it is not generally possible to incorporate this knowledge into correlative statistical models in a principled way.
...
When no single modelling approach is available that fits their needs, species distribution modellers that require more ecological mechanism in their predictions may rely on a number of tricks to shoehorn their ecological knowledge into correlative SDMs.
...
Models that explicitly consider the demographic processes relevant to the distribution modelling task in hand are therefore appealing.

Models have been proposed to improve the realism in species distribution models, whilst still enabling them to be fitted to the distribution data that are commonly available.
Dynamic range models (DRMs) extend correlative SDMs by explicitly modelling dispersal processes and spatial variation in population growth rates [@pagelForecastingSpeciesRanges2012].
...
Because they do not model vital rates or population dynamic processes directly, DRMs, like correlative SDMs, are unable to predict the impact on distributions of a change that affects a single vital rate (e.g. only survival or only fecundity).
It is also not possible to extend DRMs to incorporate important population dynamic processes other than dispersal - such as density dependence, seed-banks, biotic interactions, and evolution.

By contrast to DRMs, matrix population models and their recently developed extension integral projection models (IPMs) are demographic models of the dynamics of a population at a single location.
Both approach focus on the construct and analysis of a transition matrix (or kernel) representing key demographic processes [@merowAdvancingPopulationEcology2014].
Spatial IPMs^[Note that @merowUsingIntegralProjection2014 use the term 'demographic distribution model' to describe their spatial application of an integral projection model, however since that title could be applied to a very broad range of models including the approach introduced in this paper, we use the term spatial integral projection model to refer to their approach] extend this idea to map distributions by modelling demographic rates as a function of covariates, then analysing transition matrices under different conditions to map emergent demographic quantities (such as the intrinsic growth rate) to characterize the distribution of a species [@merowUsingIntegralProjection2014].
Spatial IPMs require data on vital rates at a number of sites, in order to be able to characterise how vital rates vary as a function of multiple spatial covariates.
Such data are unlikely to be available for modelling most species, restricting the applicability of this approach to a small subset of distribution modelling applications.

However, spatial IPMs provide a mechanistic mathematical link between environmental covariates, demography, and distributions, making it possible to perform statistical inference on these spatially-varying vital rates from distribution data.
I refer to this framework of fitting a spatially-varying MPM or IPM to distribution data as a demographic species distribution model.
This approach draws on the strengths of both DRMs and spatial IPMs (and indeed includes both as special cases), enabling the user to make use of both demographic data and distribution data, to the extent that either is available for the target species.
...
The rest of this manuscript addresses these issues, introducing both a basic model and a more complex example, and describing a number of potential extensions.

# Demographic Species Distribution Models

## Defining the model

In correlative species distribution modelling, the analyst begins by selecting a statistical or machine-learning modeling framework that is appropriate to the data type and is thought to perform well.
By contrast, the construction of a DSDM begins with the selection of a population matrix structure that is appropriate to represent relevant aspects of the population dynamics of the species, and the questions being asked of the model.
For example, in a species where vital rates strongly depend on the age or size of individuals, a matrix model representing multiple life stages structured by age or size would likely be appropriate.
If an aim of the analysis relates to the dispersal of the species (e.g. a range-shifting species), then explicit consideration of of dispersal in the momdel would be warranted.

The intention of this paper is to introduce and demonstrate a general modelling approach and related class of models, rather than proposing a single matrix structure that would be expected to apply in all cases.
In this sense, the proposed DSDM framework is akin to mathematical modelling, rather than correlative statistical modelling.
However, a number of different features of population dynamics can easily be included in DSDMs, in a modular way.
The DSDM structure given in equations 2-3 is simple version for presence-absence data which makes a number of assumptions about the species ecology.
This structure can easily be extended to accommodate alternative data types and demographic processes, and answer a range of ecological questions, by extending this basic model in a modular way.
These extensions are discussed below.
The appendix provides notation and code for fitting a number of models that can be fitted within this framework, capturing a wide range of ecological processes including: density dependence, non-stationary population dynamics, dispersal and non-equilibrium distributions, biotic interactions and evolutionary dynamics.

Whilst statistical inference against distribution data could be performed on almost any demographic model, I restrict the models discussed here to those that are both based on transition matrices, and amenable to efficient MCMC methods.
By taking a Bayesian approach, analysts are able to incorporate ecological knowledge into models, but also account for uncertainty in this knowledge and allow assumptions to be updated on fitting to distribution data.
Fitting by MCMC also fully captures uncertainty about model parameters (poor model identifiability) and propagates it into predictions, which can be important where a species' dstribution can be equally well explained by the same spatial pattern applied to one or other vital rate models.
This constrains the considered class of models to those with likelihoods that are tractable (the exact value of likelihood can be computed from the parameters and data), and can be differentiated with respect to parameter values.
This enables the use of modern gradient-based MCMC samplers [@monnahanFasterEstimationBayesian2017], though it necessarily restricts the scope of these models to those with only continuous parameters.
The model variations I propose encompass a wide range of features whilst remaining within these constraints, and can also make use of efficient vectorised and linear algebra routines, meaning that they should scale well to the large datasets commonly used for fitting SDMs.
