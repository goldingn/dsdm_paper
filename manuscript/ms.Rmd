...
\begin{align}
\mathbf{A} = \begin{pmatrix} F \cdot S_J & F \cdot S_A \\ S_J & S_A \end{pmatrix}
\end{align}

where and $F$, $S_J$, and $S_A$ are single spatial estiamtes of annual fecundity, juvenile survival, and adult survival rates respectively.
...
p_i &= 1 - e^{-\psi \Lambda_i} \\ \nonumber
\Lambda_i &= ln(1 + e^{(\lambda_i - 1) M})  \\ \nonumber
\lambda_i &= eigval(\mathbf{A_i})
\end{align}

where $eigval()$ denotes the dominant eigenvalue of a symmetric matrix, yielding the intrinsic growth rate $\lambda_i$ of the populations at location $i$. From the growth rate, the relative abundance of individuals $\Lambda_i$ is calculated from an expression that combined the carrying capacity expression from the Beverton-Holt model: $K_i = (\lambda_i - 1) M$, where $M$ quantifies how competition limits population sizes, transformed via the softplus function $ln(1 + e^{K_i})$ so that $\Lambda_i$ is positive for all parameter values. The softplus transformation is necessary since the Beverton-Holt expression would otherwise yield zero or negative relative abundances when $\lambda_i <= 1$), and can be thought of a smooth version of the Heaviside function, with $\Lambda_i$ almost identical to $K_i$ for most $lambda_i > 1$.
This formulation is statistically convenient, but as discussed in the text below, DSDMs can incorporate more complex and realistic forms of density-dependence if required.
The parameter $\psi$ is an individual-level detection rate so that $\psi \Lambda_i$ gives the expected number of individuals detected during the survey at observation $i$. This leads us to the probability $p_i$ of detecting (one or more individuals of) the species $y_i$, under a poisson sampling distribution [@pagelQuantifyingRangewideVariation2014].
...
The DSDM structure given in equations 2-3 is the simplest version. It can easily be extend to accommodate alternative data types and demographic processes, and answer a range of ecological questions.
...
An observation model for count data can be specified simply by replacing the bernoulli sampling distribution in equation 3 with an appropriatecount (such as a Poisson or negative binomial distribution) with $\Lambda_i$ as the expected count.
A model for presence-only data can be formed by considering the data as arising from a Poisson point process with $\Lambda_i$ as the intensity at location $i$ [@rennerPointProcessModels2015]. Integration areas for each observation, and any modelled or prespecified sampling bias can be provided via $\alpha$.
...
Long-term temporal trends driven by changing environmental covariates can be accommodated in this model simply be taking the index $i$ to represent observations at distinct locations *and times*, with the observation vector and design matrix extended accordingly.
Shorter-term temporal trends and the impacts of environmental stochasticity on species persistence can also be included by using of dynamic (temporally-varying) transition matrices:

\begin{align}
\Lambda_{t,i} &= \alpha N_{t,i}  \\ \nonumber
N_{t,i} &= \mathbf{A(\mathbf{X}_{t,i})}N_{t-1,i}
\end{align}

where the new variable $N_{t,i}$ gives the expected population size at time $t$, which is also used as an index to environmental covariates $\mathbf{X}$ and observations. Rather than the intrisic growth rate computed form the static matrix, this formulation explicitly calculates population size at each time $t$ from the deterministic dynamics that define the matrix population model. In this formulation of the DSDM, the initial population at each site $N_{0,i}$ must be provided. This could either be specified as a parameter in the model, via a known initial population size, or as an arbitrary fixed value accompanied by a sufficient 'burn in' period that parameter estimates are not influenced.
...
Bayesian inference on the DSDM for the Carolina Chickadee by MCMC took `r round(bbs_time)` seconds on a laptop computer. For this particular model, the computation time can be sped up by using an analytic solution to the transition matrix eigenvalues: $\lambda_i = F_i S_{Ji} + S_{Ai}$.
